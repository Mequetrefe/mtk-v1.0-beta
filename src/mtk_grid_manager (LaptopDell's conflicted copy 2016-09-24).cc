...
MTKGrid1D* MTKGridManager::MultData1D (MTK_Real mult,
                                       MTKGrid1D* grid_in) {
  ...
  for ( ii=0; ii<Mult->nNodes_+2*Mult->nDummy_; ii++)
    Mult->Grid_[ii]=
      (Mult->xmin_) - (Mult->nDummy_)*(Mult->dx_) + ii*(Mult->dx_);


  Mult->CBMetric_= (MTK_Real *)malloc(nelm*sizeof(MTK_Real));

  for (int ii =0; ii<nelm; ii++)
    Mult->CBMetric_[ii] = grid_in->CBMetric_[ii];

  Mult->Data_ = (MTK_Real*) calloc( (Mult->datanelm_),sizeof(MTK_Real) );
    if (Mult->Data_ == NULL){
...
MTKGrid1D* MTKGridManager::CGM1DDiv( MTKGrid1D* const grid_in ){
  ...
  Div->CBMetric_= (MTK_Real *)malloc(nelm*sizeof(MTK_Real));

  for (int ii=0; ii<nelm; ii++)
    Div->CBMetric_[ii] = grid_in->CBMetric_[ii];

  Div->Data_ = (MTK_Real*) calloc( (Div->datanelm_),sizeof(MTK_Real) );
  if (Div->Data_ == NULL){
    printf("Could not allocate data!!!\n");
    return nullptr;
  }
...
MTKGrid1D* MTKGridManager::Set2Sum1D (MTKGrid1D* const grid_in1,
  ...
  Output->CBMetric_= (MTK_Real *)malloc(nelm*sizeof(MTK_Real));

  for (ii=0; ii<nelm; ii++)
    Output->CBMetric_[ii] = grid_in1->CBMetric_[ii];

  Output->Data_ = (MTK_Real*) calloc( (Output->datanelm_),sizeof(MTK_Real) );
  ...
  for(ii=Output->nDummy_;(ii<Output->datanelm_-Output->nDummy_);ii++) {
    ...
    c*grid_in2->GetData1Df((MTK_Real)(ii-grid_in2->nDummy_) + shiftIndex);
  }
    return Output;
}
